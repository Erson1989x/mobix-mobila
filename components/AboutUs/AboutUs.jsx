...
const AboutUs = () => {
  ...
  return (
    ...
    <h2 className="mt-8 text-3xl lg:text-4xl"> Despre noi </h2>
    ...
    {aboutUsDataSections.map(({ title, description, content }, index) => (
          <div key={index} className="card">
            <h4 className="text-lg font-bold text-center lg:text-xl">{title}</h4>
            {description ? (
              <p className="mt-2 text-md text-center lg:text-lg">{description}</p>
            ) : (
              <p className="mt-2 text-md text-center lg:text-lg">{content}</p>
            )}
          </div>
        ))}
    </div>
